...
float4 CalculateDecalCurvedViewPos(float4 vertex) {
	...
	float zOff = vPos.y / _CurvedParam.x;
	vPos += float4(0, -_CurvedParam.y, 0, 0)*zOff*zOff;
...
float4 CalculateFacingCameraCurvedViewPos(float4 vertex) {
	...
	float zOff = viewPos.y / _CurvedParam.x;
	viewPos += float4(0, -_CurvedParam.y, 0, 0)*zOff*zOff;
